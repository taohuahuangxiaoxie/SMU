...
- **参数**：
  ...
  - `-h, --help`：显示帮助信息
- **工作流程**：
  1. 检查架构库目录：验证`lib/{arch}/`目录是否存在
  2. 创建构建目录：`build-{arch}/`
  3. 配置CMake（自动使用`lib/{arch}/`目录）
  4. 编译指定项目或所有项目
...
- **参数**：
  ...
  - `-h, --help`：显示帮助信息
- **工作流程**：
  1. 设置库路径：设置`LD_LIBRARY_PATH`指向`lib/{arch}/`目录
  2. 查找可执行文件
  3. 运行测试（支持超时控制）
  4. 统计测试结果
...
不同架构直接链接对应的架构目录，无需拷贝文件。每个架构的库文件存放在独立的`lib/{arch}/`目录中，CMake根据目标架构自动选择正确的库路径。
...
1. **编译时**：CMake根据`ARCH`变量自动使用`lib/{arch}/`目录进行链接
2. **测试时**：`test.sh`设置`LD_LIBRARY_PATH`指向`lib/{arch}/`目录，确保运行时加载正确的库文件
3. **无需拷贝**：库文件始终保持在各自的架构目录中，不会互相干扰

### 优势

- ✅ 架构隔离：不同架构的库文件完全分离，避免混淆
- ✅ 无需拷贝：减少文件操作，提高构建效率
- ✅ 清晰明确：库文件路径与架构一一对应，易于管理
- ✅ 支持并行：可以同时保留多个架构的库文件
...
- 每个架构的库文件存放在独立的`lib/{arch}/`目录中
- CMake编译时自动使用对应架构的库目录
- 测试时通过`LD_LIBRARY_PATH`环境变量加载正确的库文件
- 建议在版本控制中只保留`lib/{arch}/`目录，忽略根目录下的库文件
...
**症状**：`找不到 zstd 库文件，查找路径: /workspace/github/SMU/lib/{arch}`
